...
import (
	...
	"github.com/payshares/horizon/log"
	"github.com/payshares/horizon/paths"
	"github.com/payshares/horizon/reap"
	"github.com/payshares/horizon/render/sse"
...
// CoreSession returns a new session that loads data from the payshares core
// database. The returned session is bound to `ctx`.
...
// UpdatePaysharesCoreInfo updates the value of coreVersion and networkPassphrase
// from the Payshares core API.
func (a *App) UpdatePaysharesCoreInfo() {
	if a.config.PaysharesCoreURL == "" {
		return
	}

	fail := func(err error) {
		log.Warnf("could not load payshares-core info: %s", err)
	}

	resp, err := http.Get(fmt.Sprint(a.config.PaysharesCoreURL, "/info"))
	...
	// TODO: make resilient to changes in payshares-core's info output
...
func (a *App) Tick() {
	...
	// update ledger state and payshares-core info in parallel
	wg.Add(2)
	go func() { a.UpdateLedgerState(); wg.Done() }()
	go func() { a.UpdatePaysharesCoreInfo(); wg.Done() }()
	wg.Wait()
