...
// that submits directly to the payshares-core at `url` using the http client
// `h`.
...
// coreSubmissionResponse is the json response from payshares-core's tx endpoint
...
// submits directly to the configured payshares-core instance using the
...
// Submit sends the provided envelope to payshares-core and parses the response into
// a SubmissionResult
func (sub *submitter) Submit(ctx context.Context, env string) (result SubmissionResult) {
	...
	// interpet response
	if cresp.Exception != "" {
		result.Err = errors.Errorf("payshares-core exception: %s", cresp.Exception)
		return
	}
	...
	default:
		result.Err = errors.Errorf("Unrecognized payshares-core status response: %s", cresp.Status)
	}
