...
const (
	...
	// DefaultPaysharesCoreURL is the default postgres connection string
	// for horizon's test payshares core database.
	DefaultPaysharesCoreURL = "postgres://localhost:5432/payshares-core_test?sslmode=disable"
)
...
// PaysharesCore returns a connection to the payshares core test database
func PaysharesCore() *sqlx.DB {
	if coreDB != nil {
		return coreDB
	}
	coreDB = OpenDatabase(PaysharesCoreURL())
	return coreDB
}

// PaysharesCoreURL returns the database connection the url any test
// use when connecting to the payshares-core database
func PaysharesCoreURL() string {
	databaseURL := os.Getenv("PAYSHARES_CORE_DATABASE_URL")

	if databaseURL == "" {
		databaseURL = DefaultPaysharesCoreURL
	}
