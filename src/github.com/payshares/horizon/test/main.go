...
import (
	...
	tdb "github.com/payshares/horizon/test/db"
...
// LoadScenarioWithoutHorizon populates the test Payshares core database a with
...
func Start(t *testing.T) *T {
	...
	result.CoreDB = PaysharesCoreDatabase()
...
// PaysharesCoreDatabase returns a connection to the payshares core test database
//
// DEPRECATED:  use `PaysharesCore()` from test/db package
func PaysharesCoreDatabase() *sqlx.DB {
	return tdb.PaysharesCore()
}

// PaysharesCoreDatabaseURL returns the database connection the url any test
// use when connecting to the payshares-core database
//
// DEPRECATED:  use `PaysharesCoreURL()` from test/db package
func PaysharesCoreDatabaseURL() string {
	return tdb.PaysharesCoreURL()
}
