...
// data produced by the connected payshares-core database, transforms it and
...
// Cursor iterates through a payshares core database's ledgers
type Cursor struct {
	...
	// attempt to be ingested in this session.
	LastLedger int32
	// DB is the payshares-core db that data is ingested from.
	DB *db.Session
...
type System struct {
	...
	// CoreDB is the payshares-core db that data is ingested from.
	CoreDB *db.Session
	...
	// PaysharesCoreURL is the http endpoint of the payshares-core that data is being
	// ingested from.
	PaysharesCoreURL string
...
// New initializes the ingester, causing it to begin polling the payshares-core
// database for now ledgers and ingesting data into the horizon database.
func New(network string, coreURL string, core, horizon *db.Session) *System {
	i := &System{
		Network:        network,
		PaysharesCoreURL: coreURL,
		HorizonDB:      horizon,
		CoreDB:         core,
	}
...
func NewSession(first, last int32, i *System) *Session {
	...
	return &Session{
		...
		Network:          i.Network,
		PaysharesCoreURL:   i.PaysharesCoreURL,
