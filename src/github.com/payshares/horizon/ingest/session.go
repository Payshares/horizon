...
import (
	...
	"github.com/payshares/go/meta"
	"github.com/payshares/go/xdr"
	"github.com/payshares/horizon/db2/history"
	"github.com/payshares/horizon/ingest/participants"
)
...
func (is *Session) ingestEffects() {
	...
	case xdr.OperationTypeCreatePassiveOffer:
		...
		// KNOWN ISSUE:  payshares-core creates results for CreatePassiveOffer operations
...
func (is *Session) ingestTrades() {
	...
	for i, trade := range trades {
		// payshares-core will opportunisticly garbage collect invalid offers (in the
...
// reportCursorState makes an http request to the configured payshares-core server
// to report that it has finished processing the data being ingested.  This
// allows payshares-core to free that storage when next it runs its own
// maintenance.
func (is *Session) reportCursorState() error {
	...
	u, err := url.Parse(is.PaysharesCoreURL)
	if err != nil {
		return err
	}
	...
	if body != "Done" {
		return fmt.Errorf("failed to set cursor on payshares-core: %s", body)
	}
