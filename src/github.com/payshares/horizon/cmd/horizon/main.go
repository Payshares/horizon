...
import (
	...
	"github.com/payshares/horizon"
	hlog "github.com/payshares/horizon/log"
)
...
func init() {
	...
	viper.BindEnv("payshares-core-db-url", "PAYSHARES_CORE_DATABASE_URL")
	viper.BindEnv("payshares-core-url", "PAYSHARES_CORE_URL")
	...
	rootCmd = &cobra.Command{
		Use:   "horizon",
		Short: "client-facing api server for the payshares network",
		Long:  "client-facing api server for the payshares network",
	...
	rootCmd.Flags().String(
		"payshares-core-db-url",
		"",
		"payshares-core postgres database to connect with",
	)

	rootCmd.Flags().String(
		"payshares-core-url",
		"",
		"payshares-core to connect with (for http commands)",
	)
	...
	rootCmd.Flags().Bool(
		"ingest",
		false,
		"causes this horizon process to ingest data from payshares-core into horizon's db",
	)
	...
	rootCmd.Flags().Uint(
		...
		0,
		"the maximum number of ledgers the history db is allowed to be out of date from the connected payshares-core db before horizon considers history stale",
	)
...
func initConfig() {
	...
	if viper.GetString("payshares-core-db-url") == "" {
		log.Fatal("Invalid config: payshares-core-db-url is blank.  Please specify --payshares-core-db-url on the command line or set the PAYSHARES_CORE_DATABASE_URL environment variable.")
	}

	if viper.GetString("payshares-core-url") == "" {
		log.Fatal("Invalid config: payshares-core-url is blank.  Please specify --payshares-core-url on the command line or set the PAYSHARES_CORE_URL environment variable.")
	}
	...
	config = horizon.Config{
		...
		PaysharesCoreDatabaseURL: viper.GetString("payshares-core-db-url"),
		PaysharesCoreURL:         viper.GetString("payshares-core-url"),
