...
import (
	...
	"github.com/payshares/horizon/db2/schema"
...
func ingestSystem() *ingest.System {
	...
	cdb, err := db.Open("postgres", config.PaysharesCoreDatabaseURL)
	if err != nil {
		log.Fatal(err)
	}
	...
	i := ingest.New(passphrase, config.PaysharesCoreURL, cdb, hdb)
	return i
}
