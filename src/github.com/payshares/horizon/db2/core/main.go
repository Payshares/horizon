...
// reading rows from a Payshares Core db
package core
...
// Offer is row of data from the `offers` table from payshares-core
type Offer struct {
...
// Q is a helper struct on which to hang common queries against a payshares
// core database.
type Q struct {
	*db.Session
}
...
// Signer is a row of data from the `signers` table from payshares-core
type Signer struct {
	Accountid string
	Publickey string
	Weight    int32
...
// Transaction is row of data from the `txhistory` table from payshares-core
...
// TransactionFee is row of data from the `txfeehistory` table from payshares-core
...
// Trustline is a row of data from the `trustlines` table from payshares-core
...
// payshares-core database this ingestion system is communicating with.  Horizon,
...
// database.
//
// Due to the design of payshares-core, ledger 1 will _always_ be in the database,
