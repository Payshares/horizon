...
using JavaScript and `EventSource`.  We will eschew using [`js-payshares-sdk`](https://github.com/payshares/js-payshares-sdk), the
...
```javascript
var Keypair = require("payshares-base").Keypair;
...
The Payshares test network provides the Friendbot, a tool that developers
can use to get testnet stakks for testing purposes. To fund your account, simply
execute the following curl command:

```bash
$ curl "https://horizon-testnet.payshares.org/friendbot?addr=GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
After a few seconds, the Payshares network will perform consensus, close the
...
```bash
$ curl -H "Accept: text/event-stream" "https://horizon-testnet.payshares.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments"
```
...
> If you need a reliable streaming connection please use our [SDK](https://github.com/payshares/js-payshares-sdk).
...
var es = new EventSource('https://horizon-testnet.payshares.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments');
...
```bash
$ curl "https://horizon-testnet.payshares.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
var keypair = PaysharesBase.Keypair.fromSeed('SCU36VV2OYTUMDSSU4EIVX4UUHY3XC7N44VL4IJ26IOG6HVNC7DY5UJO');
var account = new PaysharesBase.Account(keypair.accountId(), "713226564141056");
...
var transaction = new PaysharesBase.TransactionBuilder(account)
  .addOperation(PaysharesBase.Operation.payment({
    destination: PaysharesBase.Keypair.random().accountId(),
    asset: asset,
    amount: amount
  }))
  .addSigner(keypair)
...
After running this script you should see a signed transaction blob. To submit this transaction we send it to horizon or payshares-core. But before we do, let's open a new console and start our previous script by `node stream_payments.js`.
